  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const categoryList = Object.keys(grouped).map((title) => ({ title }));

        setCategories(categoryList);
        setProductsByCategory(grouped);
        if (categoryList.length > 0) setActiveTab(categoryList[0].title);
      } catch (err) {
  ...
  return (
    <section className="w-full py-10 bg-gradient-to-tr from-sky-400 to-blue-600">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 px-4 md:px-10">
        <h2 className="text-2xl font-bold mb-4 md:mb-0 text-white">Similar Products</h2>
        <div className="flex gap-4 overflow-x-auto no-scrollbar">
          {categories.map((cat) => (
            <button
              key={cat.title}
              onClick={() => setActiveTab(cat.title)}
              className={`pb-1 font-medium ${activeTab === cat.title
        ...
        .map((cat) => {
          ...
          if (!products.length) {
            return (
              <p
                key={cat.title}
                className="text-white text-center py-6 text-lg bg-blue-500 rounded-lg"
              >
                No products found in {cat.title}.
              </p>
            );
          }
